...
# # 	lambda2 = true_eigenvalues[n-2]
# # 	x_true = true_eigenvectors[:,[n-1]]
# 	# print(f"l1certo: {lambda1} l# 2certo: {lambda2}")
# 	# for i in range(1, it_max):
# 		x_values.ap# pend(i)
# 		omega = metodo.opt# imal_omega(lambda1)
# 		x_k = me# todo.update_x_k()
# 		mu_k = metodo.update_mu_k()
# 		eigenva# lue_error = get_eigenvalue_error(mu_k, lambda1)
# 		print(f"va# lerr: {eigenvalue_error} = {mu_k} - {lambda1}")
# 		eigenve# ctor_error = get_eigenvector_error(x_k, x_true)
# 		#print(f"vec_error: {ei# genvector_error}\nxk:\n{x_k}\nxtrue\n{x_true}")
# 		e# igenvalue_error_vector.append(eigenvalue_error)
# 		eig# envector_error_vector.append(eigenv# ector_error)
# 		if ei# genvector# _error <= epsilon:
# 			converged = True
# 			break
# 	assint_values, assint_values_squared = get_assintotico# (lambda1, lambda2, x_values[len(x_values) - 1])
...
# 	# print(f"Matrix: \n{A}\nx_0:\n{vector_x0}\nmu_k: {mu# _k} l1: {lambda1}\nx_k:\n{x_k}\nx*:\n{x_true}")
# 	# print(f"l1: {la# mbda1} l2: {lambda2} l2/l1: {lambda2/lambda1}")
# 	# print(f"o# mega: {omega} m# u_k: {mu_k} lambda1: {lambda1}")
# 	if converged:# 
# 		prin# t(f"\nConverged before reaching it_max")
# 	else:
# 		print# (f"\nDid not converge before reaching it_max")

# 	plot_aproximations(x_values, eigenvalue_error_vector, eigenvector_erro# r_vector, assint_values, assint_values_squared)
